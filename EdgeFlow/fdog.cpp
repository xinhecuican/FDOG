...
int GetDirectionalDoG(imatrix& image, ETF& e, mymatrix& dog, myvec& GAU1, myvec& GAU2, double tau)
{
	myvec vn(2);
	double x, y, d_x, d_y;
...
void GetFlowDoG(ETF& e, mymatrix& dog, mymatrix& tmp, myvec& GAU3, mymatrix& weights)
{
	myvec vt(2);
	double x, y, d_x, d_y;
...
void GetFDoG(imatrix& image, ETF& e, double sigma, double sigma3, double tau, bool inhibition)
{
	int	i, j;
	...
	mymatrix weights(image_x, image_y);
	imatrix mask(image_x, image_y);
	mymatrix dog2(image_x, image_y);

	int max_dog = GetDirectionalDoG(image, e, dog, GAU1, GAU2, tau);
	GetFlowDoG(e, dog, tmp, GAU3, weights);
	if (inhibition)
	{
		for (int i = 1; i < image_x - 1; i++)
		{
			for (int k = 1; k < image_y - 1; k++)
			{
				if (tmp[i][k] == 1)
				{
					continue;
				...
				//	for (int n = -1; n <= 1; n++)
				//	{
				//		strength[m + 1][n + 1] = sqrt(pow(e[i+m][k+n].tx, 2) + pow(e[i+m][k+n].ty, 2));
				//	}
				//}
				if (tx == 0)
				{
				...
				{
					float tan_theta = ty / tx;
					float tan_theta2 = tan_theta != 0 ? 1 / tan_theta : 0;

					if (tan_theta >= 0 && tan_theta <= 1)
					{
						p1 = (1 - tan_theta2) * weights[i][k-1] + tan_theta2 * weights[i-1][k-1];
						p2 = (1 - tan_theta2) * weights[i][k+1] + tan_theta2 * weights[i+1][k+1];
						
					}
					else if (tan_theta > 1)
					{
						p1 = (1 - tan_theta) * weights[i+1][k] + tan_theta * weights[i+1][k+1];
						p2 = (1 - tan_theta) * weights[i-1][k] + tan_theta * weights[i-1][k-1];
					}
				...
				if (weights[i][k] < p1 || weights[i][k] < p2)
				{
					tmp[i][k] = 1;
				}
			}
